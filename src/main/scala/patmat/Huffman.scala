...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def loop(clist: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
      clist match {
        case Nil => acc
        case x :: xs =>
    ...
    def loop(b: List[Bit], t: CodeTree, acc: List[Char]): List[Char] = {
      b match {
        ...
        case x :: xs => {
          ...
          node match {
            case Leaf(c, w) => c :: loop(xs, tree, acc)
            case Fork(l, r, c, w) => loop(xs, node, acc)
          }
        }
      }
    }
...
//    case Leaf(c, w) => ???
    case Fork(l, r, c, w) => if (b == 0) l else r
  }
  ...
  def recConvert(tree: CodeTree, currentBitList: List[Bit]): CodeTable = {
    tree match {
      case Fork(leftTree, rightTree, _, _) => {
        mergeCodeTables(recConvert(leftTree, 0 :: currentBitList),
          recConvert(rightTree, 1 :: currentBitList))
      }
